...
adc = np.load('ppg_rock_fast.npz', allow_pickle=True)['arr_0'].item()['adc'][400:]
# adc = adc - np.mean(adc)
print(len(adc))
...
plt.clf()
plt.plot(np.arange(len(adc))/400, adc, label="ADC")
plt.xlabel('Time (s)')
plt.ylabel('Adc (V)')
plt.legend()
plt.savefig("adc_rock_fast.png")

freq, adc_fft = fft(adc, 4096, 400)
freq_range = (freq >= 0.5) & (freq <= 2.0)  
