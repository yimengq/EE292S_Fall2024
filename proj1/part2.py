...
if __name__ == '__main__':
    ...
    R = np.array([[0.01]])
    # process noise
    Q = 5e-4*np.array([[1., 0., 0.],
                  [0., 1., 0.],
                  [0., 0., 1.]])
    # Q = np.full((3,3), 10.)
    # covariance matrix
    P = 5e-2*np.array([[1.,  0., 0.],
                 [ 0., 1., 0.],
                 [ 0., 0., 1.]])
    ...
    try:
        while True:
            ...
            if len(accelx_lst) < 500:
                accelx_lst.append(accx)
                time_base = currTime
                if len(accelx_lst) == 499:
                    print("calibration done")
            else:
                accelx_avg = sum(accelx_lst)/500
        ...
        np.savez('data_6.npz', tn=tn, pos_kal=pos_kal, vel_kal=vel_kal, accel_kal=accel_kal, pos_original=pos_original, vel_original=vel_original, accel_original=accel_original)
        ...
        plt.legend()
        plt.savefig('acceleration_plot_6.png')  # Save acceleration plot
        ...
        plt.legend()
        plt.savefig('vel_plot_6.png')  # Save acceleration plot
        plt.close()  # Close the current figure
        ...
        plt.legend()
        plt.savefig('pos_plot_6.png')  # Save acceleration plot
        plt.close()  # Close the current figure
